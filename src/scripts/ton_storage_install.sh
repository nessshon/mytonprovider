...
compilation() {
	...
	CGO_ENABLED=1 ${go_path} build -o ${bin_path} ${src_path}/cli/main.go
}

ton_storage_setup(){
	echo -e "${COLOR}[1/4]${ENDC} Cloning Ton-Storage repository"
	clone_repository
	...
	echo -e "${COLOR}[4/4]${ENDC} Ton-Storage installation complete"
}
