...
compilation() {
	...
	CGO_ENABLED=1 ${go_path} build -o ${bin_path} ${src_path}/main.go
}

ton_tunnel_provider_setup(){
	echo -e "${COLOR}[1/4]${ENDC} Cloning Ton-Tunnel-Provider repository"
	clone_repository
	...
	echo -e "${COLOR}[4/4]${ENDC} Ton-Tunnel-Provider installation complete"
}
