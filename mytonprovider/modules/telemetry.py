...
class TelemetryModule(
    ...
    def cycle_send(self) -> None:
        ...
        if not state.enabled:
            self.logger.debug("telemetry disabled, skipping cycle")
            return
        try:
            self.api.send_telemetry(self._build_telemetry())
            self.logger.debug(f"telemetry sent to {constants.TELEMETRY_URL}")
        except Exception:
        ...
        if now - state.last_benchmark_sent_at < self.BENCHMARK_INTERVAL_SEC:
            remaining = state.last_benchmark_sent_at + self.BENCHMARK_INTERVAL_SEC - now
            self.logger.debug(f"benchmark not due yet ({utils.format_duration(remaining)} until next)")
            return
        bench_payload = self._build_benchmark()
        if bench_payload is None:
            self.logger.debug("benchmark payload empty, skipping send")
            return
        try:
            self.api.send_benchmark(bench_payload)
            state.last_benchmark_sent_at = now
            self.logger.debug(f"benchmark sent to {constants.BENCHMARK_URL}")
        except Exception:
    ...
    def _cmd_enable(self, app: Any, _args: list[str]) -> None:
        ...
        # x2 tolerates one missed cycle before considering telemetry dead
        state = self.app.db.modules.telemetry
        cycle_alive = (int(time.time()) - state.last_cycle_at) < self.TELEMETRY_INTERVAL_SEC * 2
        if not cycle_alive:
            self.app.modules.get_by_class(MytonproviderModule).service.restart()
    ...
    def _build_telemetry(self) -> TelemetryPayload:
        ...
        sm = self.app.modules.get_by_class(SysMetricsModule)
    ...
    def _build_provider(self) -> TelemetryProvider:
        tsp = self.app.modules.get_by_class(TonStorageProviderModule)
    ...
    def _build_benchmark(self) -> BenchmarkPayload | None:
        bench = self.app.modules.get_by_class(BenchmarkModule)
        if bench.snapshot is None:
            return None
        tsp = self.app.modules.get_by_class(TonStorageProviderModule)
        snap = bench.snapshot
