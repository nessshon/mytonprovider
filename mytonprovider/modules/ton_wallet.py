...
from mypycli.types import BoxStyle, Color, ColorText, Command, StatusPanel
...
class TonWalletModule(
    ...
    MIN_BALANCE: Final[float] = 0.2
    GAS_RESERVE: Final[float] = 0.0005
    ...
    def _cmd_transfer(self, app: Any, args: list[str]) -> None:
        ...
        needed_nano = amount_nano + to_nano(self.GAS_RESERVE)
    ...
    def _cmd_register(self, app: Any, args: list[str]) -> None:
        ...
        try:
            msg_hash = self.run_async(
                self.send_and_wait(
                    Address(constants.REGISTRATION_DESTINATION),
    ...
    def _provider(self) -> TonStorageProviderModule:
        return self.app.modules.get_by_class(TonStorageProviderModule)
    ...
    async def _init_ton_client(self) -> None:
        cfg = self.app.db.settings.ton_api
