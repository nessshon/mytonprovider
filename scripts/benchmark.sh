...
test_path="${1}"
test_file="${test_path}/test.img"
test_db="${test_path}/bench"
mkdir -p ${test_path}
...
# lite
lite_result=$(fio --name=test --runtime=60 --readwrite=randrw --blocksize=4k --ioengine=libaio --direct=1 --size=4G --filename=${test_file} --rwmixread=75 --randrepeat=1 --gtod_reduce=1 --iodepth=64)
lite_json_result=$(get_fio_json "$lite_result")

# hard
hard_result=$(fio --name=test --runtime=60 --readwrite=randrw --blocksize=4k --ioengine=libaio --direct=1 --size=4G --filename=${test_file} --rwmixread=75 --io_size=10g --fsync=1 --iodepth=1 --numjobs=1)
hard_json_result=$(get_fio_json "$hard_result")

# full
full_result=$(/usr/bin/db_bench --benchmarks="randomtransaction" -max_background_flushes 2 -max_background_compactions 4 -bytes_per_sync 1048576 -writable_file_max_buffer_size 32768 -duration 60 -threads 8 -db=${test_db} 2>/dev/null)
