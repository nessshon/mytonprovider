...
check_go_version() {
	go_mod_path=${1}
	go_path=${2}
	need_version_text=$(grep "^go " ${go_mod_path} | head -n 1 | awk '{print $2}')
	need_version_3=$(echo ${need_version_text} | cut -d "." -f 3)
	if [ -z "${need_version_3}" ]; then
		need_version_text="${need_version_text}.0"
	fi
	if [ ! -f ${go_path} ]; then
		install_go ${need_version_text}
		return
	fi
	...
	need_version=$(echo ${need_version_text} | cut -d "." -f 1,2)
	current_version=$(echo ${current_version_text} | cut -d "." -f 1,2)
...
install_go() {
	...
	go_url=https://go.dev/dl/go${go_version}.linux-${arc}.tar.gz
	rm -rf /usr/local/go
